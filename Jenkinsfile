...
pipeline {
    ...
    triggers {
       GenericTrigger(
            ...
            //token: 'GSR2GCSTERKUVZU5TMTAOAGQ8FWEJ',
            token: '6TU4GGCHC8RMCI9OQKSYI5Z4PAMJD',
    ...
    stages {
        ...
        //checkout scm
            steps {
                catchError {
                    ...
                    userRemoteConfigs: [[url: 'https://ilhamgrip@bitbucket.org/grip-batam/smart-apes-frontend.git',credentialsId:'ffdd87eb-a4a9-4d31-b8ef-c8de22f22868']]
                    ])
                }
            }
        }
        ...
        //this will build 2 image with tag Build ID number and tag latest
            steps {
                echo "Build new Docker Image : Smartapes-Frontend-marketplace"
                        ...
                        if(env.VERSION != 'no-build'){
                            if (env.DEPLOY == 'staging' && env.TARGET == "master"){
                                ...
                                docker.withRegistry('https://registry.digitalocean.com', 'GripRegistry') {
                                    withCredentials([string(credentialsId: 'doppler-secret-key-smartapes-marketplace-staging', variable: 'dopplerServiceKeyStaging')]) {
                                    def buildApp = docker.build("grip/smartapes-marketplace:${env.VERSION}", "--build-arg DOPPLER_TOKEN=${dopplerServiceKeyStaging} --build-arg REACT_APP_VERSION=${env.VERSION} .")
                                    // withCredentials([string(credentialsId: 'doppler-secret-staging-smartapes', variable: 'dopplertokenaccess')]) {
                                    //     def buildApp = docker.build("grip/smartapes-marketplace:${env.VERSION}", "--build-arg DOPPLER_TOKEN=${dopplertokenaccess} --build-arg REACT_APP_VERSION=${env.VERSION} .")
                                ...
                                echo "Build new Docker Image : smartapes-marketplace:${env.VERSION}"
                                echo "Build Docker Image because this is staging Pipeline"
                                docker.withRegistry('https://registry.digitalocean.com', 'GripRegistry') {
                                    def buildApp = docker.build ("grip/smartapes-marketplace:${env.VERSION}")
                                    stage ('Publish Docker') {
                                        buildApp.push('latest')
                                    }
                                }
                                ...
                                docker.withRegistry('https://registry.digitalocean.com', 'GripRegistry') {
                                    def buildApp = docker.build("grip/smartapes-marketplace:${env.VERSION}","-f Dockerfile.prod .")
                                    stage ('Publish Docker') {
                                        buildApp.push()
                                    }
                                }
                    ...
                    script{                                                
                        if(env.VERSION != 'no-build'){
                            if (env.DEPLOY == 'staging' && env.TARGET == "master"){
                                ...
                                withCredentials([usernamePassword(credentialsId: 'ssh-access-smartapes-staging', passwordVariable: 'password', usernameVariable: 'userName'), string(credentialsId: 'doppler-secret-key-smartapes-marketplace-staging', variable: 'dopplerServiceKeyStaging')]) {
                                    remoteStage.user = userName
                                    remoteStage.password = password
                                    sshCommand remote: remoteStage, command: "cd ~; if cd docker-compose; then git pull; else git clone https://steven-grip:aJuD4QwY2BzZy7pS5Muw@bitbucket.org/grip-batam/docker-compose.git; fi; cd SMARTAPES-PLATFORM/staging/frontend-marketplace; docker-compose pull; docker-compose down -v; DOPPLER_TOKEN_SMA_FRONTEND_MARKETPLACE=${dopplerServiceKeyStaging} docker-compose up -d"
                                }
                            }else if (env.DEPLOY == 'staging' && env.TARGET == "sandbox"){
                                echo "Deploy to Sandbox "
                                withCredentials([usernamePassword(credentialsId: '86ec2886-3964-488b-8f1b-df1724106a0a', passwordVariable: 'password', usernameVariable: 'userName'), string(credentialsId: 'doppler-secret-key-smartapes-marketplace-sanbox', variable: 'dopplerServiceKeyStaging')]) {
                                    remotesandbox.user = userName
                                    remotesandbox.password = password
                                    sshCommand remote: remotesandbox, command: "cd ~; if cd docker-compose; then git pull; else git clone https://steven-grip:aJuD4QwY2BzZy7pS5Muw@bitbucket.org/grip-batam/docker-compose.git; fi; cd SMARTAPES-PLATFORM/sandbox/frontend-marketplace; docker-compose pull; docker-compose down -v; DOPPLER_TOKEN_SMA_FRONTEND_MARKETPLACE=${dopplerServiceKeyStaging} docker-compose up -d"
                                }
                            }
                            else if (env.DEPLOY == 'production' ){
                                ...
                                sh "cat smartapes-app-helm/templates/frontend-marketplace/deployment.yaml"
                                sh "sed -i 's+registry.digitalocean.com/grip/smartapes-marketplace.*+registry.digitalocean.com/grip/smartapes-marketplace:${env.VERSION}+g' smartapes-app-helm/templates/frontend-marketplace/deployment.yaml"
                                sh "cat smartapes-app-helm/templates/frontend-marketplace/deployment.yaml"
                                sh "git add ."
